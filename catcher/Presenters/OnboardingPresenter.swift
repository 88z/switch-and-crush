...
class OnboardingPresenter: BattleFieldPresenter {
    ...
    override func present(){
        ...
        battleFieldScene?.start(level: Level(initialObstacleTypes: [], initialSpeed: 200, acceleration: 0.00, name:"1", initialState: State.first, userInterationEnabled: false, colorScheme: .defaultScheme()), shouldShowCounter: false)
    ...
    override func uiScenePressed(scene: TrivialUIScene) {
        if scene == switchColorUI {
            vc?.hideUI()
            battleFieldScene?.start(level: Level(initialObstacleTypes: [.plank(blinkInterval: 0)], initialSpeed: 200, acceleration: 0.00, name:"1", initialState: State.second, userInterationEnabled: false, colorScheme: .defaultScheme()), shouldShowCounter: false)
            let frame = vc?.view.frame ?? .zero
            ...
            vc?.hideUI()
            battleFieldScene?.start(level: Level(initialObstacleTypes: [.plank(blinkInterval: 0)], initialSpeed: 200, acceleration: 0.00, name:"1", initialState: State.first, userInterationEnabled: false, colorScheme: .defaultScheme()), shouldShowCounter: false)
            let frame = vc?.view.frame ?? .zero
