...
class OnboardingPresenter: BasePresenter {
    ...
    override func present(){
        ...
        let thisIsBallUI = OneActionScene(size: UIScreen.main.bounds.size, elements:[
            ...
            OneActionSceneText(position: CGPoint(x: frame.midX, y: frame.minY + 200), delayBeforePresent: 3, text: "Tap anywhere to switch its color"),
    ...
    override func oneActionScenePressed(scene: OneActionScene) {
        if scene == thisIsBallUI {
            vc?.hideUI()
            battleFieldScene?.start(level: Level(obstacleCount: 1, initialSpeed: 3, acceleration: 0.00, name:"1", initialState: State.first, userInterationEnabled: false))
